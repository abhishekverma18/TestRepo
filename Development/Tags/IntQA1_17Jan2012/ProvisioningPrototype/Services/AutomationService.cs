    ...
    public class AutomationService
    {
        ...
        public LinkInfo SetUpContext(PanelPreferences preferences, string skinFolderPath)
        {
            ...
            if (!preferences.OfflineMode)
            {
                ...
                try
                {
                   // Added by K.G(07-12-2011) attempt to Navigate away to attempt not to lock panel settings
                    ClosePanelSettings(preferences);
        ...
        // Added By K.G(07/12/2011) to find the eid of created template
        private string FindEid(string pageResource, string etemp)
        {
            int indexTempName = pageResource.LastIndexOf(etemp);
            string eidSubstring = pageResource.Substring(indexTempName + etemp.Length);
            int indexofEid = eidSubstring.IndexOf("eid:");
            string eid = eidSubstring.Substring(indexofEid + "eid:".Length);
            string[] eidArray = eid.Split(',');
            return eidArray[0];
        }
        ...
        public bool UploadSkins(PanelPreferences preferences, string skinFolderPath, string newFolderName)
        {
            ...
            string newSkinPackagePath = Path.Combine(skinFolderPath, Res.NewSkinPackagePath);
            CookieJar = UploadSkinsToAssetManager(newPortalSkinPath, newSurveySkinpath, newSkinPackagePath, preferences, newFolderName);
        ...
        private CookieJar UploadSkinsToAssetManager(string newPortalSkinPath, string newSurveySkinPath, string newSkinPackagePath, PanelPreferences preferences, string newFolderName)
        {
            // Modified by K.G(07/12/2011)  current location is www.quicksetup.visioncriticalpanels.com/Admin/CommunicationsManagementView.aspx so we need to navigate from CommunicationsManagementView.aspx to AssetManagerView.aspx
            //PanelSettingsManagement.PanelSettingsPostToAssetManager(preferences);
            Communication.CommunicationPostToAssetManager(preferences);
            ...
            CookieJar = AssetManager.AssetManagerUploadNewSkinPackage(CookieJar, newSkinPackagePath, preferences, newFolderName);
