...
namespace ProvisioningPrototype
{
    public class ContextCollection
    {
        ...
        public List<ContextInfo> GetAllContextInfo()
        {
            List<ContextInfo> lstContextInfo = new List<ContextInfo>();
            foreach (var context in ContextList)
            {
                foreach (var contextItem in context.ContextItems)
                {
                    if (contextItem.PanelSettings.Any(panelSetting => panelSetting.PanelSettingName.Equals("OpenPortalSkinFolder")))
                    {
                        ContextInfo contextInfo = GetContextInfo(contextItem.PanelSettings, context.ContextItems[2].PanelSettings, context.ContextIndex);
                        lstContextInfo.Add(contextInfo);
                    }
                }
            }
            return lstContextInfo;

        }
        
        private String readSetting(List<PanelSetting> pathsPanelSettings, String settingName, bool throwOnMissing)
        {
            var matchingItems = pathsPanelSettings.Where(setting => setting.PanelSettingName.Equals(settingName));
            var result = matchingItems.FirstOrDefault();
            if (result != null) 
            { 
                return result.Value;
            }
            else if (throwOnMissing) { 
                throw new Exception(settingName + "not found in Paths Panel Settings");
            }
            return String.Empty;
        }
