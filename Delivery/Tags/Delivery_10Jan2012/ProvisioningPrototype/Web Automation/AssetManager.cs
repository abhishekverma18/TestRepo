...
namespace ProvisioningPrototype.Web_Automation
{
    public class AssetManager
    {
        ...
        public static CookieJar AssetManagerUploadNewSkinPackage(CookieJar cookieJar, string path, PanelPreferences preferences,string newFolderName)
        {
            var fileData = new FileStream(path, FileMode.Open, FileAccess.Read);

            var request = (HttpWebRequest)WebRequest.Create(preferences.PanelAdminUrl + "upload.axd?up=p:" + newFolderName);
            ...
            stringBuilder.AppendFormat("--{0}\r\n", boundary);
            stringBuilder.AppendFormat("Content-Disposition: form-data; name=\"{0}\"\r\n\r\n{1}\r\n", "Filename", Res.NewSkinPackagePath);
            stringBuilder.AppendFormat("--{0}\r\n", boundary);
            stringBuilder.AppendFormat("Content-Disposition: form-data; name=\"{0}\";filename=\"{1}\"\r\n", "Filedata", Res.NewSkinPackagePath);
            stringBuilder.AppendFormat("Content-Type: {0}\r\n\r\n", "application/octet-stream");
